...
public class SaveSaltField : MonoBehaviour {
	...
	void Start(){
		...
		if (File.Exists (Application.persistentDataPath + "/playerInfoSaltField.dat")) {
			BinaryFormatter bf = new BinaryFormatter ();
			FileStream playerInfo = File.Open (Application.persistentDataPath + "/playerInfoSaltField.dat", FileMode.Open);
			PlayerDataSaltField playerInfoData = (PlayerDataSaltField)bf.Deserialize (playerInfo);
			playerInfo.Close ();
		...
		if (File.Exists (Application.persistentDataPath + "/ResearchTimeSaltField.dat")){
			BinaryFormatter bf = new BinaryFormatter ();
			FileStream researchTime = File.Open (Application.persistentDataPath + "/ResearchTimeSaltField.dat", FileMode.Open);
			PlayerDataSaltField researchTimeData = (PlayerDataSaltField)bf.Deserialize (researchTime);
			researchTime.Close ();
	...
	void OnApplicationPause(){
		BinaryFormatter bf = new BinaryFormatter ();
		FileStream file = File.Create (Application.persistentDataPath + "/playerInfoSaltField.dat");
		PlayerDataSaltField data = new PlayerDataSaltField ();
		FileStream gold = File.Create (Application.persistentDataPath + "/Gold.dat");
		Gold goldData = new Gold();
	...
	public void WhenClicked(){
		BinaryFormatter bf = new BinaryFormatter ();
		FileStream file = File.Create (Application.persistentDataPath + "/ResearchTimeSaltField.dat");
		PlayerDataSaltField data = new PlayerDataSaltField ();
