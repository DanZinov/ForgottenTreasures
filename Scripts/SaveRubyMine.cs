...
public class SaveRubyMine : MonoBehaviour {
	...
	void Start(){
		...
		if (File.Exists (Application.persistentDataPath + "/playerInfoRubyMine.dat")) {
			BinaryFormatter bf = new BinaryFormatter ();
			FileStream playerInfo = File.Open (Application.persistentDataPath + "/playerInfoRubyMine.dat", FileMode.Open);
			PlayerDataRubyMine playerInfoData = (PlayerDataRubyMine)bf.Deserialize (playerInfo);
			playerInfo.Close ();
		...
		if (File.Exists (Application.persistentDataPath + "/ResearchTimeRubyMine.dat")){
			BinaryFormatter bf = new BinaryFormatter ();
			FileStream researchTime = File.Open (Application.persistentDataPath + "/ResearchTimeRubyMine.dat", FileMode.Open);
			PlayerDataRubyMine researchTimeData = (PlayerDataRubyMine)bf.Deserialize (researchTime);
			researchTime.Close ();
	...
	void OnApplicationPause(){
		BinaryFormatter bf = new BinaryFormatter ();
		FileStream file = File.Create (Application.persistentDataPath + "/playerInfoRubyMine.dat");
		PlayerDataRubyMine data = new PlayerDataRubyMine ();
		FileStream gold = File.Create (Application.persistentDataPath + "/Gold.dat");
		Gold goldData = new Gold();
	...
	public void WhenClicked(){
		BinaryFormatter bf = new BinaryFormatter ();
		FileStream file = File.Create (Application.persistentDataPath + "/ResearchTimeRubyMine.dat");
		PlayerDataRubyMine data = new PlayerDataRubyMine ();
